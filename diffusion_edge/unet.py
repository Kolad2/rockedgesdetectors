...
class SwinFeatureExtractor(nn.Module):
    """Swin-B stages with the parameter names used by DiffusionEdge."""

    def __init__(self):
        super().__init__()
        backbone = swin_b(weights=None)
        layers = list(backbone.features.children())
        self.first_coonv = layers[0]
        self.features = nn.ModuleList(layers[1:])
        # These modules exist in the released checkpoint even though feature
        # extraction does not call them.
...
class SpatialAtt(nn.Module):
    ...
    def forward(self, x):
        ...
        q = q.permute(0, 2, 3, 1).reshape(b, h * w, 1)
        k = self.k_conv(att)
        k = k.reshape(b, 1, h * w)
        att = att.permute(0, 2, 3, 1).reshape(b, h * w, 1)
...
class WeightStandardizedConv2d(nn.Conv2d):
    ...
    def forward(self, x):
        ...
        mean = weight.mean(dim=dimensions, keepdim=True)
        var = weight.var(dim=dimensions, keepdim=True, unbiased=False)
...
class LinearAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        q, k, v = (
            tensor.reshape(b, self.heads, -1, h * w)
            for tensor in qkv
        )
    ...
    def __init__(
        ...
        **kwargs
        ...
        if cfg.cond_net != 'swin':
            raise ValueError("The released BSDS model requires the Swin-B backbone")
        f_condnet = 128
        self.init_conv_mask = SwinFeatureExtractor()
        self.init_conv = nn.Sequential(
