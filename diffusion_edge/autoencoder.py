...
class LinearAttention(nn.Module):
    ...
    def forward(self, x):
        b, c, h, w = x.shape
        qkv = self.to_qkv(x)
        channels = qkv.shape[1] // (3 * self.heads)
        qkv = qkv.reshape(b, 3, self.heads, channels, h * w)
        q, k, v = qkv.permute(1, 0, 2, 3, 4)
        k = k.softmax(dim=-1)
        context = torch.einsum('bhdn,bhen->bhde', k, v)
        out = torch.einsum('bhde,bhdn->bhen', context, q)
        out = out.reshape(b, self.heads * channels, h, w)
...
class AutoencoderKL(nn.Module):
    def __init__(self,
        ...
        # The perceptual/adversarial loss belongs to training only.  Keeping it
        # out makes the released inference model depend on PyTorch alone.
        self.loss = None
